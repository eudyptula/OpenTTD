		...
		for (int dy = -3; dy <= (height-1) + 3; dy++) {
			if (CMSAIndustry(TILE_MASK(tile + TileDiffXY(dx, dy)))) {
				...
				if (IsUniqueStationName(buf)) {
					free(st->name);
					st->name = stredup(buf, lastof(buf));
					return true;
				}
			}
		}
...
static CommandCost CheckFlatLandAirport(AirportTileTableIterator tile_iter, DoCommandFlag flags, StationID *station)
{
	...
	for (; tile_iter != INVALID_TILE; ++tile_iter) {
         ...
         * in *station */
        if (station != NULL && IsTileType(tile_iter, MP_STATION)) {
            if (!IsAirport(tile_iter)) {
                return ClearTile_Station(tile_iter, DC_AUTO); // get error message
            } else {
                StationID st = GetStationIndex(tile_iter);
